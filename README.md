...
┌──────────────────┐     ┌─────────────────────────────────────────────┐     ┌───────────┐
│                  │     │              Google Cloud Run               │     │           │
│  Mobile Browser  │────▶│                                             │────▶│   ESP32   │
│  (index.html)    │     │  Node.js + Express + ws                     │     │  GPIO 2   │
│                  │     │  - Serves static frontend                   │     │ (built-in │
│                  │     │  - WebSocket server (wss)                   │◀────│   LED)    │
│                  │     │  - REST API (/api/led, /api/verify)         │     │           │
│                  │     │  - Google OAuth verification                │     └───────────┘
│                  │     │  - Async Google Sheets logging               │
│                  │     └──────────┬──────────────────────────────────┘
│                  │                │
│  Android App     │                │ HTTPS / Google Sheets API
│  (ESP32LedApp)   │                │ (Application Default Credentials)
│                  │     ┌──────────▼──────────────────────────────────┐
│                  │     │              Google Sheets                  │
│                  │     │  ┌──────────────┐  ┌───────────────────┐  │
│                  │     │  │ allowed_users │  │       logs        │  │
│                  │     │  │  (email col)  │  │ (timestamp, user, │  │
│                  │     │  └──────────────┘  │  action, status,   │  │
│                  │     │                    │  response_time_ms) │  │
│                  │     │                    └───────────────────┘  │
│                  │     └───────────────────────────────────────────┘
...
└────────┬────────┘
    ...
    └───┬───┬────┘    │                            │
        ...
        │     ▼                                    │
        │ ┌──────────────────────────────────────┐ │
        │ │ 5 seconds since last reconnect?      │ │
        │ └───┬───────────────┬──────────────────┘ │
        │     │ Yes           │ No                 │
        │     ▼               │                    │
        │ ┌──────────────────┐│                    │
...
```
  ┌───────────┐         ┌──────────────────┐         ┌──────────┐         ┌──────────────┐
  │  Mobile   │         │  Cloud Run       │         │  ESP32   │         │  Google      │
  │  Browser  │         │  (Node.js)       │         │          │         │  Sheets      │
  └─────┬─────┘         └────────┬─────────┘         └────┬─────┘         └──────┬───────┘
        │                       │                        │                      │
        │                       │     WSS connect        │                      │
        │                       │◄────────────────────────┤                      │
